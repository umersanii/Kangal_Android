...
class TransactionsDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<Transaction>> getAllTransactions(int limit, int offset) =>
      (select(transactionsTable)..limit(limit, offset: offset)).get();

  Future<Transaction?> getTransactionById(int id) => (select(
    transactionsTable,
  )..where((t) => t.id.equals(id))).getSingleOrNull();

  Future<Transaction?> getTransactionByTransactionId(String txnId) => (select(
    transactionsTable,
  )..where((t) => t.transactionId.equals(txnId))).getSingleOrNull();

  Future<int> insertTransaction(Insertable<Transaction> companion) =>
      into(transactionsTable).insert(companion);

  Future<bool> updateTransaction(Insertable<Transaction> companion) =>
  ...
  Future<List<Transaction>> getTransactionsBySource(String source) =>
      (select(transactionsTable)..where((t) => t.source.equals(source))).get();

  Future<List<Transaction>> searchTransactions(String query) =>
      (select(transactionsTable)..where((t) => t.note.like('%$query%'))).get();

  Future<List<Transaction>> getUnsyncedTransactions() =>
      (select(transactionsTable)..where((t) => t.syncedAt.isNull())).get();
}
