...
| Task | Description | Completed | Date |
| -------- | ----------- | --------- | ---- |
| TASK-021 | Create `lib/data/services/secure_storage_service.dart`: class `SecureStorageService` wrapping `FlutterSecureStorage`. Methods: `saveEmailCredentials(String email, String appPassword)`, `getEmailCredentials() → Future<({String email, String appPassword})?>`, `deleteEmailCredentials()`, `saveSupabaseToken(String token)`, `getSupabaseToken()`, `deleteSupabaseToken()`, `hasEmailCredentials() → Future<bool>`. All keys prefixed with `kangal_`. Commit message: `feat: add secure storage service for credentials management` | ✅ | 2026-03-13 |
...
| TASK-044 | Write unit tests: `test/data/services/auto_categorisation_service_test.dart` (keyword matching, case insensitivity, no match returns null, first match wins), `test/ui/settings/categories/categories_view_model_test.dart` (CRUD operations, default category deletion prevented, reassignment on delete), `test/ui/settings/rules/rules_view_model_test.dart` (CRUD operations, bulk apply). Commit message: `test: add unit tests for categories, rules, and auto-categorisation` ✅ | 2026-03-13 |
...
| TASK-057 | Create `lib/data/models/sync_result.dart` (freezed): fields `int uploaded`, `int downloaded`, `int conflictsResolved`, `bool success`, `String? errorMessage`. Commit message: `feat: add SyncResult data model` | ✅ | 2026-03-15 |
| TASK-058 | Create `lib/data/repositories/sync_repository.dart`: abstract class `SyncRepository` with methods `syncNow() → Future<SyncResult>`, `getLastSyncTime() → Future<DateTime?>`, `hasUnsyncedChanges() → Future<bool>`. Create `lib/data/repositories/sync_repository_impl.dart`: concrete implementation using `SupabaseSyncService` and `SyncLogDao`. `hasUnsyncedChanges()` checks for transactions where `syncedAt IS NULL` or `updatedAt > syncedAt`. Commit message: `feat: implement sync repository with sync status queries` | ✅ | 2026-03-15 |
| TASK-059 | Extend `BackgroundSyncService` (from TASK-028) to also run Supabase sync. Add periodic task `supabase_sync` with 30-minute frequency, constraint: `NetworkType.connected`. The callback creates `SupabaseSyncService` and calls `syncAll()`. Only runs if user is authenticated (check via `SupabaseAuthService.isAuthenticated()`). Commit message: `feat: add Supabase background sync to workmanager schedule` | ✅ | 2026-03-15 |
...
- **FILE-004**: `lib/data/database/app_database.dart` — Drift `AppDatabase` class with all tables/DAOs and `beforeOpen` seeding hook for default categories
...
- **FILE-008**: `lib/data/database/tables/sync_log_table.dart` — Drift table matching PRD §4.4 schema (`syncTableName` mapped to `table_name`)
- **FILE-009**: `lib/data/database/daos/transactions_dao.dart` — CRUD + queries mapped to/from `TransactionModel` (ISO date string persistence)
- **FILE-010**: `lib/data/database/daos/categories_dao.dart` — CRUD mapped to/from `CategoryModel`
- **FILE-011**: `lib/data/database/daos/rules_dao.dart` — CRUD mapped to/from `RuleModel`
- **FILE-012**: `lib/data/database/daos/sync_log_dao.dart` — Sync log queries/upsert mapped to/from `SyncLogModel`
- **FILE-013**: `lib/data/models/transaction_model.dart` — Abstract Freezed immutable transaction model
- **FILE-014**: `lib/data/models/category_model.dart` — Abstract Freezed immutable category model
- **FILE-015**: `lib/data/models/rule_model.dart` — Abstract Freezed immutable rule model
- **FILE-016**: `lib/data/models/sync_log_model.dart` — Abstract Freezed immutable sync log model
- **FILE-017**: `lib/data/models/sync_result.dart` — Freezed sync result model
- **FILE-018**: `lib/data/repositories/transaction_repository.dart` — Abstract interface using `TransactionModel`
- **FILE-019**: `lib/data/repositories/drift_transaction_repository.dart` — Drift-backed implementation using `TransactionModel`
...
- **FILE-022**: `lib/data/repositories/rule_repository.dart` — Abstract interface using `RuleModel`
- **FILE-023**: `lib/data/repositories/drift_rule_repository.dart` — Drift-backed implementation using `RuleModel`
